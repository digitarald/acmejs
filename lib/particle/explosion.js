...
Explosion = (function(_super) {
  ...
  Explosion.prototype.update = function(dt, scene) {
    var age, dist, factor, i, impulse, kinetic, max, particle, radius, radiusSq, _i, _j, _len, _ref;
    if (this.state === 'began') {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        Vec2.add(kinetic.force, Vec2.scal(Vec2.norm(Vec2.sub(kinetic.pos, this.pos, Vec2.cache[0])), max * factor));
      }
      impulse = Vec2.cache[0];
      for (i = _j = 0; _j <= 100; i = _j += 1) {
        Vec2.norm(Vec2.set(impulse, Math.rand(-1, 1), Math.rand(-1, 1)), null, Math.rand(0, max));
        particle = Particle.alloc(this.root, this.pos, impulse, Math.rand(this.lifetime / 2, this.lifetime * 2), Math.rand(1, 4), 0);
      }
