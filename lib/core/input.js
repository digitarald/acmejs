...
Input = (function(_super) {
  ...
  Input.prototype.handleEvent = function(event) {
    ...
    if (throttled[type] != null) {
      if (throttled[type] >= 0) {
        this.queue[throttled[type]] = event;
        return this;
      }
      throttled[type] = this.queue.length;
    } else {
  ...
  Input.prototype.lateUpdate = function(dt, scene) {
    ...
    if (event) {
      if (this.queue.length) {
        console.log(this.queue.map(function(evt) {
          return event.type;
        }));
      }
      if (this.throttled[event.type] != null) {
        this.throttled[event.type] = -1;
      }
