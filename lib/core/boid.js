...
Boid.fixedUpdate = function(dt) {
  ...
  while (i--) {
    ...
    if (cohesionCount && boid1.cohesionMod) {
      ...
      Vec2.add(entity1.kinetic.acc, Vec2.scal(cohesion, boid1.cohesionMod * mod));
    }
    if (imitationCount && boid1.imitationMod) {
      ...
      Vec2.add(entity1.kinetic.acc, Vec2.sub(acc, vel));
    }
    if (avoidanceCount && boid1.avoidanceMod) {
      if (avoidanceCount > 1) {
        Vec2.scal(avoidance, 1 / avoidanceCount);
      }
      Vec2.sub(entity1.kinetic.acc, Vec2.scal(avoidance, boid1.avoidanceMod * mod));
    }
  }
  return this;
};
