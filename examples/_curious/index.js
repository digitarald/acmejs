...
var Bounds, Collider, Color, Comet, Component, Earth, Context, Entity, GameController, Body, MenuController, Particle, Registry, Renderer, Sprite, Transform, Vec2, Weapon,
...
new Registry(MenuController);

Context.menuScene = Entity.create(null, {
  menuController: null
});
...
GameController = (function(_super) {
  ...
  GameController.prototype.create = function() {
    ...
    Earth.Prefab.create(this.root, {
      transform: {
        position: Vec2(240, 160)
      }
    });
    return this;
  };
  ...
  GameController.prototype.update = function(dt) {
    ...
    comet = Comet.Prefab.create(this.entity, {
      transform: {
        position: pos
      },
      bounds: {
    ...
    vel = comet.body.velocity;
    Vec2.add(vel, Vec2(Math.rand(0.8, 1.2), Math.rand(0.9, 1.1)));
    return Vec2.scale(vel, Math.rand(30, 40));
  };
...
Earth = (function(_super) {
  ...
  Earth.prototype.fixedUpdate = function(dt) {
    var distSq, factor, gravityRadiusSq, body, pos, pos2, _i, _len, _ref, _results;
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      body = _ref[_i];
      if (!(body.enabled && !body.fixed && body.mass)) {
        continue;
      }
      pos2 = body.entity.transform.position;
      distSq = Vec2.distSq(pos, pos2);
      if (distSq < gravityRadiusSq) {
        factor = 1 - Math.sqrt(distSq) / this.gravityRadius;
        _results.push(body.applyForce(Vec2.norm(Vec2.sub(pos, pos2, Vec2.cache[0]), null, Math.quadIn(factor) * 750 / body.mass)));
      } else {
  ...
  Earth.prototype.update = function(dt) {
    var dist, input;
    input = Context.input;
    Vec2.sub(input.position, this.transform.position, this.normal);
    switch (this.state) {
      case null:
      ...
      case 'active':
        ...
        if (input.touchState === 'ended') {
          Weapon.Prefab.create(this.root, {
            transform: {
              position: Vec2.add(Vec2.rotate(Vec2.set(Vec2.cache[0], this.bounds.radius * 1.2, 0), this.angle), this.transform.position),
              angle: this.angle
            }
          });
          this.state = null;
...
Comet = (function(_super) {
  ...
  Comet.prototype.create = function(attributes) {
    var center;
    this.body.maxVelocity = 100;
    this.age = 0;
    this.target = Earth.pool.instances[0].transform;
  ...
  Comet.prototype.update = function(dt) {
    ...
    this.body.mass = this.bounds.radius;
    ...
    while (i--) {
      ...
      particle = Entity.create(this.root, {
        transform: {
          position: pos
        },
        body: {
          velocity: Vec2.scale(pointer, Math.rand(-0.5, -4))
        },
        particle: {
  ...
  Comet.prototype.render = function(ctx) {
    if (Context.renderer.cull(this)) {
      return this;
    }
    ctx.beginPath();
    ctx.arc(this.transform.position[0], this.transform.position[1], this.bounds.radius, 0, Math.TAU);
    ctx.closePath();
  ...
  Comet.prototype.explode = function() {
    ...
    while (i--) {
      ...
      particle = Particle.Prefab.create(this.root, {
        transform: {
          position: pos
        },
        body: {
          velocity: Vec2.scale(pointer, Math.rand(1, 5))
        },
        particle: {
          radius: radius,
...
Weapon = (function(_super) {
  ...
  Weapon.prototype.create = function(attributes) {
    this.state = null;
    this.arc = Math.TAU / 8;
    this.targets = Comet;
    this.range = 100;
  ...
  Weapon.prototype.update = function(dt) {
    ...
    if (this.transform.angle !== this.angle) {
      Vec2.rotate(Vec2.set(pos, this.orbit, 0), this.angle);
      Vec2.copy(this.normal, pos);
      Vec2.add(pos, this.entity.transform.position);
      this.transform.angle = this.angle;
    }
    rangeSq = this.range * this.range;
    switch (this.state) {
      case null:
        if (Vec2.distSq(input.position, pos) < this.rangeSq) {
          rad = Vec2.rad(Vec2.sub(input.position, pos, normal), this.normal);
          if (rad < this.arc / 2) {
            console.log('locked');
            this.locked = input.position;
            this.state = 'locked';
            debugger;
          }
        }
        break;
      case 'locked':
        Vec2.sub(input.position, pos, this.lockedNorm);
  ...
  Weapon.prototype.intercept = function(target, targetVel) {
    ...
    return Vec2.scale(Vec2.sub(interception, pos, Vec2()), vel / Math.sqrt(Vec2.dot(dist, dist)));
  };
